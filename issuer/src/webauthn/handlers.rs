...
fn compute_subject_hash(local_handle: &str) -> String {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"webauthn:subject:v1:");
    hasher.update(local_handle.as_bytes());
    hasher.finalize().to_hex().to_string()
}

/// Compute authentication proof for Sybil resistance.
/// This proof is bound to an opaque subject hash rather than a user-facing
/// label, so issuance does not need to carry a local passkey handle.
...
pub fn router(state: Arc<WebAuthnState>) -> Router {
    ...
    let base_router = if use_attestation {
        Router::new()
            .route("/", get(webauthn_app))
            .route("/app", get(webauthn_app))
            .route("/register", get(webauthn_app))
            .route("/authenticate", get(webauthn_app))
...
pub async fn webauthn_app() -> Html<&'static str> {
    Html(include_str!("../../../webauthn-client/index.html"))
}
...
#[instrument(
    name = "webauthn_start_registration",
...
pub async fn start_registration(
    ...
    tracing::Span::current().record("subject_hash", tracing::field::display(&subject_hash));
    ...
    let existing_creds = state
        .cred_store
        .load_user_credentials(&subject_hash)
        .await
    ...
    if !existing_creds.is_empty() {
        ...
        // Generic error message to prevent local-handle enumeration
        // Don't reveal whether the user exists or how many credentials they have
        return Err((
            StatusCode::CONFLICT,
            "Registration not available for this local handle".to_string(),
        ));
    }
    ...
    // webauthn-rs 0.5.3 API: start_passkey_registration(uuid, name, display_name, exclude)
...
pub async fn finish_registration(
    ...
    let (reg_state, _local_handle, subject_hash, client_ip) = {
        let mut sessions = state.sessions.write().await;
    ...
    state
        .cred_store
        .save(cred_id.clone().into(), passkey, subject_hash.clone())
        .await
        .map_err(|e| {
...
#[instrument(
    name = "webauthn_start_authentication",
    skip(state, connect_info, headers),
    fields(subject_hash)
)]
pub async fn start_authentication(
    ...
    let subject_hash = compute_subject_hash(&local_handle);
    tracing::Span::current().record("subject_hash", tracing::field::display(&subject_hash));
    debug!(subject_hash = %subject_hash, "Starting WebAuthn authentication");
    ...
    if stored_creds.is_empty() {
        debug!(subject_hash = %subject_hash, "No credentials found for subject (returning generic error)");
...
pub async fn finish_authentication(
    ...
    let (auth_state, subject_hash, client_ip) = {
        let mut sessions = state.sessions.write().await;
        match sessions.remove(&req.session_id) {
    ...
    let proof = compute_auth_proof(&state.webauthn.rp_id, &subject_hash, authenticated_at);
